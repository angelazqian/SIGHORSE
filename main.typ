...
#set heading(numbering: "1.1")
#show heading: set text(tracking: -0.25pt)
...
/////////////////////////////////////////////////
#block(spacing: 0.5em, inset: (right: 0.46in, left: 0.46in))[
...
#let ponysona = [#pad([#figure(
  pad(
  image("images/ponysona.png", width: 54%),
  left: 1.5em,
  bottom: -1em
  ),
...
left:-8em, right: -8.5em, top:-2em, bottom:1em
)]
//Horse representation \ of this project. Unclear if \ author has ever seen a horse.
// Author's artistic interpretation of if this project was a horse. Unclear if author has ever seen a horse.
// Author's artistic \ interpretation of if this project \ was a horse. Unclear if \ author has ever seen a horse.
#wrap-content(
  ponysona,
  [
...
) <img:highscore>], left:-4.8em, right: -4.5em, top:-0.45em, bottom:0.5em)]
...
#grid(
  ...
  [#pad([#figure(
  pad(
  image("images/preference.png", width: 47%),
  top: 1.0em,
  bottom: -0.5em,
  right: -1.5em
  ),
  caption: [My preferred \ orientation],
  ) <img:preference>], left:-4.5em)],
  ...
  ) <img:mirrots>], left:-3.5em, right:1em, bottom: 0em, top:0.35em)],
)
...
#let mixup = [#pad([#figure(
  ...
  right: -0.3em, bottom: -0.22em
  ),
  caption: [Ambiguity leads to all four \ directions being possible "good moves"],
) <img:mixup>],
left: 1.15em, right: 1em, bottom: -10em, top: 0.15em
)]

#wrap-content(mixup, [
The main issue about my model at this point is that it dies a lot early game, but if it somehow survives past a certain point, it starts performing well, which I think is because of rotation noise. This is because early on in the game, the model seems to execute moves from various rotations, as if it can't decide which one to follow, leading me to remove the early gameplay from all rotations. I also noticed that when the board gains a large tile in a corner, it becomes ambiguous to the model as to which orientation it should follow. Considering that one of the main rule-of-thumb's when playing 2048 is that you should pick a direction, label it as "evil" and *avoid it at all costs*, this becomes a bit of a problem. For example, if the largest tile is placed in the top-right corner, it _could_ follow the orientation that favors the upper edge, designates `down` to be the "evil move", and mostly play moves `up`, `right`, and `left`. However, it could _also_ follow the orientation that favors the right edge, designates `left` as the "evil move", and mostly play moves `right`, `down`, and `up`. Following this logic, all 4 directions may seem like reasonable moves to the model, which is _very bad_. 
],
align: bottom + right,
)
...
[#figure(
  pad(
  image("images/flipsave.png", width: 70%),
  right: -0.4em,
  bottom: -0.5em
  ),
...
// #figure(
//   image("images/comparison.png", width: 30%),
//   caption: [#lorem(7)]
// ) <figure1>

// #figure(
//   image("images/finetune.png", width: 80%),
//   caption: [#lorem(7)]
// ) <figure2>
